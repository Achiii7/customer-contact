...
SYSTEM_PROMPT_NOTICE_SLACK = """
    ...
    - 【メンション先の選定理由】は、以下の条件に従って生成してください。
        - メンションされた従業員が選ばれた理由を、以下の情報を参考に具体的に説明してください：
            - 過去の問い合わせ対応履歴
            - 現在の業務内容や対応カテゴリ
            - 他の従業員との違いや専門性の有無
    ...
    【メンション先の選定理由】
    ・（例）○○さんは過去にも類似した問い合わせ対応実績があり、今回の質問内容と一致する対応カテゴリを担当しています。
    ・（例）□□さんは現在、該当サービスのサポート業務を主に担当しており、技術的な問い合わせに対応可能です。
    ・（例）△△さんは前回のアップデートに関する社内報の執筆を担当していたため、最新情報を熟知している可能性があります。
